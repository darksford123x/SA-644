...
func (dq *DeviceQuery) QueryRepairInformation() *RepairInvoiceQuery {
	query := &RepairInvoiceQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeviceQuery) WithRepairInformation(opts ...func(*RepairInvoiceQuery)) *DeviceQuery {
	query := &RepairInvoiceQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withRepairInformation = query
	return dq
}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context) ([]*Device, error) {
	...
	if query := dq.withRepairInformation; query != nil {
		...
		query.Where(predicate.RepairInvoice(func(s *sql.Selector) {
			s.Where(sql.InValues(device.RepairInformationColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.device_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "device_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "device_id" returned %v for node %v`, *fk, n.ID)
			}
